...
bool removeFile(const char* vFileName, int vDirInodeNum){
	SDirectory TempDirectory = loadDirectoryFromDisk(vDirInodeNum);
	short InodeIndex = findFileInodeNum(vFileName, TempDirectory);
	SBitMap InodeBitMap;
	createEmptyBitMap(InodeBitMap, g_NumInodes);
	...
	SInode TempInode = loadInodeFromDisk(InodeIndex);
	clearBitAt(InodeIndex, InodeBitMap);
	if (TempInode.FileType == 'd'){
		SInode temp;
		SDirectory dir = loadDirectoryFromDisk(InodeIndex);
		for (int i = 0; i < g_MaxNumFiles; ++i) {
			if (dir.FileSet[i].IsInUse) {
				short tempIndex = findFileInodeNum(dir.FileSet[i].FileName, dir);
				temp = loadInodeFromDisk(tempIndex);
				deallocateDisk(temp, DataBlockBitMap);
			}
		}
		deallocateDisk(TempInode, DataBlockBitMap);
		printf("移除 %s 中的文件:%s\n", TempDirectory.FileName, vFileName);
	}
	else {
		deallocateDisk(TempInode, DataBlockBitMap);
		printf("移除 %s 中的文件：%s\n", TempDirectory.FileName, vFileName);	
